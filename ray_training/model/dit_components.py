...
import sys
sys.path.append("/data0/teja_codes/ImmersoAiResearch/FlowModelTraining/miniDiffusion/src")
...
# model/dit_components.py
import math
import torch
import torch.nn.functional as F

def get_timestep_embeddings(timesteps: torch.Tensor,
                            embedding_dim: int,
                            max_period: float = 10000.0) -> torch.Tensor:
    """
    Sinusoidal timestep embeddings.
    Args:
        timesteps: (B,) or (B,1) int/float tensor
        embedding_dim: output feature size
        max_period: base for frequencies
    Returns:
        (B, embedding_dim) on the same device/dtype as timesteps (float)
    """
    # ensure shape (B,)
    if timesteps.ndim > 1:
    ...
    dtype  = torch.float32  # compute in fp32 for numeric stability

    half = embedding_dim // 2
    # arange on the SAME DEVICE
    freq_exponents = torch.arange(0, half, device=device, dtype=dtype) / half
    freqs = torch.exp(-math.log(max_period) * freq_exponents)  # (half,)
    ...
    if embedding_dim % 2 == 1:
        emb = F.pad(emb, (0, 1))  # odd dim support
