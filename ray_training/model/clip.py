...
class CLIP(nn.Module):
    ...
    def encode_text(self, input_ids, attention_mask=None):
        """Tokenize (if needed) and encode texts, returning embeddings and mask."""
        
        # Tokenize only if input is raw text
        if attention_mask is None:
            if isinstance(input_ids, str):
                ...
                raise ValueError(f"Unsupported input type for encode_text: {type(input_ids)}")
...
def load_clip(model: CLIP, model_2: OpenCLIP, device = "cpu"):
    ...
    clip_path = os.path.join(current_dir, "encoders", "hub", "checkpoints", "clip_model.pth")
    model.load_state_dict(torch.load(clip_path, map_location = device), strict = not DEBUG)
    
    path = os.path.join(current_dir, "encoders", "hub", "checkpoints", "clip2.pth")
